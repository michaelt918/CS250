...
_char_loop:
    ...
    beq $s1, $zero, _preferred_name_decision_b
    beq $s0, $zero, _preferred_name_decision_a
    
    # Compare characters
    blt $s0, $s1, _preferred_name_decision_b
    bgt $s0, $s1, _preferred_name_decision_a
...
_preferred_name_decision_a:
    ...
    lw $s3, 68($s0)  #this line loads the next list node into $s0
    j _traverse_loop


_preferred_name_decision_b:
    # Restore callee-saved registers from stack
    lw $s0, 0($sp)
    lw $s1, 4($sp)
    lw $s2, 8($sp)
    ...
    j _set_max  # Jump to set max if b is longer
